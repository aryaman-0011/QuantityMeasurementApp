 ...
 *
 * Notes: 1. Angular frontend runs on localhost:4200 2. Backend runs on
 * localhost:8080 3. Quantity APIs are permitAll() for now so frontend
 * integration works easily 4. Later we can secure them again with JWT
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
	...
	private static final String[] PUBLIC_PATHS = { "/", "/auth/**", "/oauth2/**", "/login/oauth2/code/**",
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers(PUBLIC_PATHS).permitAll()
						.requestMatchers("/api/v1/quantities/**").permitAll().requestMatchers("/api/v1/users/**")
